...
class MultiHeadedAttention(nn.Module):
    ...
    def __init__(self, n_head, n_feat, dropout_rate, n_in=None):
        ...
        self.h = n_head
        self.linear_q = nn.Linear(n_in, n_feat)
        self.linear_k = nn.Linear(n_in, n_feat)
...
class RelPositionMultiHeadedAttention(MultiHeadedAttention):
    ...
    def __init__(self, n_head, n_feat, dropout_rate, zero_triu=False, n_in=None):
        """Construct an RelPositionMultiHeadedAttention object."""
        super().__init__(n_head, n_feat, dropout_rate, n_in=n_in)
        self.zero_triu = zero_triu
        # linear transformation for positional encoding
        if n_in is None:
            n_in = n_feat
        self.linear_pos = nn.Linear(n_in, n_feat, bias=False)
        # these two learnable bias are used in matrix c and matrix d
        # as described in https://arxiv.org/abs/1901.02860 Section 3.3
        self.pos_bias_u = nn.Parameter(torch.Tensor(self.h, self.d_k))
        self.pos_bias_v = nn.Parameter(torch.Tensor(self.h, self.d_k))
